...
# Create application title and file uplader widget.
...
def load_model():
    ...
    net = cv2.cnn.readNetFromCaffe(config_file, model_file)
    return net
    

# Function to generate a dowload link for output file.
...
if img_file_buffer is not None:
    ...
    # Or use PIL Image (which uses a RGB channel order)
    # image = np.array(Image.opne(img_file_buffer))
    
    # Create placeholders to display imput and putput images.
    placeholders = st.columns(2)
    # Display Imput image in the first placeholder.
    ...
    detections = detect_faces_opencv_dnn(net, image)
    
    # Process the detections based on the current confidence threshold.
    out_image, _ = process_detection(image, detections, conf_threshold=conf_threshold)
    ...
    st.markdown(get_image_download_link(out_image, "face_output.jpg", "Downloas Output Image"), unsafe_allow_html=True)